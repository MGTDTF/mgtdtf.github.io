async function init() {
  ...
  ColorFlag = [];
  gNoArray = [];
  pImg = ["/images/plas1.gif", "/images/plas2.gif"];
  mImg = ["/images/minas1.gif", "/images/minas2.gif"];
  ...
  dImg = ["/images/tg3.gif", "/images/tg4.gif"];
  rImg = ["/images/tg1.gif", "/images/tg2.gif"];

  // Create the span element for 'TopItem'
  const span = document.createElement('span');
  span.classList.add('TopItem');
  span.onclick = AllClose; // Set the onclick event
  span.innerHTML = "<font style='font-size:11pt;'><p id='Ttree'></p><br></font>";
  
  // Append the span to the body or another container element
  document.body.appendChild(span);

  // Add the second p element
  const pTtree2 = document.createElement('p');
  pTtree2.id = "Ttree2";
  document.body.appendChild(pTtree2);

  // Call other functions as per original code
  GetModelName("Ttree");

  const xmlFile = "/xml/pdf.xml";
  let xmlDoc = await loadXML(xmlFile);
  HomePath = "";

  try {
      xmlDoc = xmlDoc.getElementsByTagName("pdf")[0];
